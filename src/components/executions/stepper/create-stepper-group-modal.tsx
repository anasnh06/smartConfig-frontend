...
// ✅ Utiliser union propre pour `order` sans erreurs
...
// Sélecteur avancé pour les serveurs
...
// Sélecteur avancé pour les templates
function TemplateSelect({ templates, value, onChange }: {
  ...
  // Correction : Template a un seul role (optionnel)
  const roleOptions = useMemo(
...
// Sélecteur avancé pour les configurations
...
export function CreateStepperGroupModal() {
  const { isCreateExecutionGroupModalOpen, closeCreateExecutionGroupModal } = useStore()
  const { addDraftGroup } = useStepperStore()
  ...
  return (
    <Dialog open={isCreateExecutionGroupModalOpen} onOpenChange={closeCreateExecutionGroupModal}>
      <DialogContent className="max-w-4xl w-full max-h-[90vh] overflow-y-auto bg-background rounded-xl shadow-2xl">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold mb-2">Ajouter un groupe d'exécution</DialogTitle>
        </DialogHeader>
        ...
        <FormProvider {...methods}>
          <form onSubmit={methods.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="flex justify-end pt-2">
              <Button type="submit" className="px-8 py-2 text-lg font-semibold">Ajouter le groupe</Button>
            </div>
          </form>
        </FormProvider>
      </DialogContent>
